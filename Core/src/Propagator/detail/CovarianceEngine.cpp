...
namespace {		
  ...
  using CurvilinearState = std::tuple<CurvilinearParameters, Jacobian, double>;
	...
	const FreeToBoundMatrix surfaceDerivative(StepperState& state, const Surface* surface = nullptr)
	{
	...
	/// @note The surface is only required for bound parameters since it serves to derive the jacobian from it. In the case of curvilinear parameters this is not needed and can be evaluated without any surface.
     void reinitializeJacToGlobal(StepperState& state, const Surface* surface = nullptr)
     {
	 ...
	 void
	 reinitializeJacobians(StepperState& state, const Surface* surface = nullptr) { 
...
namespace CovarianceEngine {
  ...
  BoundState 
  boundState(StepperState& state, const Surface& surface, bool reinitialize) {
    ...
    BoundState result = std::make_tuple(std::move(parameters), state.jacobianStepWise,
  ...
  CurvilinearState curvilinearState(StepperState& state, bool reinitialize) {
    ...
    // Create the bound state
    CurvilinearState result = std::make_tuple(std::move(parameters), state.jacobianStepWise,
                               state.pathAccumulated);
    // Reinitialize if asked to do so
    // this is useful for interruption calls
    if (reinitialize) {
  ...
  void covarianceTransport(StepperState& state, bool reinitialize, const Surface* surface) {
