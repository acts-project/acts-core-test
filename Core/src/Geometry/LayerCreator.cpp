...
Acts::MutableLayerPtr Acts::LayerCreator::cylinderLayer(
  ...
  // Remaining layer parameters - with enevlope
  double layerR = protoLayer.medium(binR, true);
  double layerThickness = protoLayer.range(binR, true);
  double layerZ = protoLayer.medium(binZ, true);
  double layerHalfZ = 0.5 * protoLayer.range(binZ, true);
...
Acts::MutableLayerPtr Acts::LayerCreator::discLayer(
  ...
  double layerZ = protoLayer.medium(binZ, true);
  double layerThickness = protoLayer.range(binZ, true);
  ...
  // Create the  disc bounds
  auto dBounds 
    = std::make_shared<const RadialBounds>(
        protoLayer.min(binR,true),
        protoLayer.max(binR,true));
...
Acts::MutableLayerPtr Acts::LayerCreator::planeLayer(
  ...
  double layerThickness = protoLayer.range(bValue, true);
  double layerHalf1 = 0.;
  double layerHalf2 = 0.;

  switch (bValue) {
    case BinningValue::binX: {
      layerHalf1 = 0.5 * protoLayer.range(binY, true);
      layerHalf2 = 0.5 * protoLayer.range(binZ, true);
      break;
    }
    case BinningValue::binY: {
      layerHalf1 = 0.5 * protoLayer.range(binX, true);
      layerHalf2 = 0.5 * protoLayer.range(binZ, true);
      break;
    }
    default: {
      layerHalf1 = 0.5 * protoLayer.range(binX, true);
      layerHalf2 = 0.5 * protoLayer.range(binY, true));
    }
  }

  double centerX = 0.5 * protoLayer.medium(binX, true);
  double centerY = 0.5 * protoLayer.medium(binY, true);
  double centerZ = 0.5 * protoLayer.medium(binZ, true);
  ...
  // Create the bounds and the layer
...
bool Acts::LayerCreator::checkBinning(const GeometryContext& gctx,
  ...
  // Consistency check: can we access all sensitive surfaces
