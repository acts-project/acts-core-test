    ...
    const Surface* surface) const
{
  ...
  for (auto& tuple_state : state.stateCol) {
    ...
    if (!surface->isOnSurface(singlestate.geoContext,
                              ...
                              EigenStepperType::direction(singlestate),
                              true)) {
      status = false;
    }
...
template <typename B, typename C, typename E, typename A>
void
Acts::MultiEigenStepper<B, C, E, A>::releaseComponents(State& state) const
{
  for (auto& tuple_state : state.stateCol) {
    ...
    const Corrector& navCorr) const -> SurfaceIntersection
{
  ...
  for (auto& tuple_state : state.stateCol) {
    ...
    if (std::get<2>(tuple_state) != StateStatus::Free) continue;
    auto& singlestate = std::get<0>(tuple_state);
    auto  target      = EigenStepperType::targetSurface(
...
template <typename B, typename C, typename E, typename A>
void
Acts::MultiEigenStepper<B, C, E, A>::updateStepSize(State& state,
                                                    ...
                                                    bool   release) const
{
  for (auto& tuple_state : state.stateCol) {
    ...
    if (std::get<2>(tuple_state) != StateStatus::Free) continue;
    auto& singlestate = std::get<0>(tuple_state);
    EigenStepperType::updateStepSize(
