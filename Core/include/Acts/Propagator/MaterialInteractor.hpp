...
struct MaterialInteractor {
 ...
 private:
  ...
  /// @brief This function applies the changes to the state due to multiple scattering
  ///
  /// @tparam propagator_state_t is the type of Propagagor state
  ///
  ...
  template <typename propagator_state_t>
  void applyMS(propagator_state_t& state, double sigmaPhi2, double sigmaTheta2) const
  {
		  ...
		  state.stepping.cov(eTHETA, eTHETA) +=
			  state.stepping.navDir * sigmaTheta2;
		} else {
  ...
  template <typename propagator_state_t, typename stepper_t>
  void applyEnergyLoss(propagator_state_t& state, const stepper_t& stepper, double deltaP, double sigmaQoP2) const
  {
       // Update the state/momentum
      stepper.update(state.stepping, stepper.position(state.stepping),
                     stepper.direction(state.stepping),
                     std::copysign(stepper.momentum(state.stepping) - deltaP, stepper.momentum(state.stepping)),
