...
struct MaterialInteractor {
  ...
  void operator()(propagator_state_t& state, const stepper_t& stepper,
    ...
    if (state.navigation.currentSurface &&
      ...
      MaterialProperties mProperties = detail::evaluateMaterialProperties(state, stepper);
      ...
      if (mProperties) {
		...
		std::array<double, 4> sigma = detail::evaluateMaterialInteraction(state, stepper, mProperties, multipleScattering, energyLoss);
 ...
 //~ /// @return Array containing the changes to the variance of phi, theta, the momentum and the variance of QoP
  //~ template <typename propagator_state_t, typename stepper_t>
  //~ std::array<double, 4> evaluateMaterialInteraction(propagator_state_t& state, const stepper_t& stepper, const MaterialProperties& mProperties) const
  //~ {
