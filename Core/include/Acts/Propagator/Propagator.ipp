...
auto Acts::Propagator<S, N>::propagate(
  ...
  if (result.ok()) {
    ...
    {
		auto curvState = m_stepper.template buildState<true, ReturnParameterType>(state.stepping, true);
	...
	{
		auto curvState =  m_stepper.template buildState<false, ReturnParameterType>(state.stepping, true);
		auto& curvParameters = std::get<ReturnParameterType>(curvState);
		// Fill the end parameters
		propRes.endParameters = std::make_unique<const ReturnParameterType>(
			std::move(curvParameters));
		...
		if (state.stepping.covTransport) {
			...
			{
			  auto& tJacobian = std::get<const FreeToBoundMatrix>(curvState);
			  propRes.transportJacobian =
				  std::make_unique<const FreeToBoundMatrix>(std::move(tJacobian));
			}
			else
			{
				auto& tJacobian = std::get<const FreeMatrix>(curvState);
			  propRes.transportJacobian =
				  std::make_unique<const FreeMatrix>(std::move(tJacobian));
			}
		}
	}
    return result;
...
auto Acts::Propagator<S, N>::propagate(
  ...
  if (result.ok()) {
	...
	else
	{
		...
		// Only fill the transport jacobian when covariance transport was done
		if (state.stepping.covTransport) {
		  auto& tJacobian = std::get<const FreeToBoundMatrix>(bs);
		  propRes.transportJacobian =
			  std::make_unique<const FreeToBoundMatrix>(std::move(tJacobian));
		}
	}
