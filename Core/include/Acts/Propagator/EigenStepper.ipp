...
auto Acts::EigenStepper<B, C, E, A>::targetSurface(
...
template <typename B, typename C, typename E, typename A>
void Acts::EigenStepper<B, C, E, A>::updateStepSize(State& state,
...
template <typename B, typename C, typename E, typename A>
void Acts::EigenStepper<B, C, E, A>::releaseStep(State& state,
...
Acts::Result<double> Acts::EigenStepper<B, C, E, A>::step(
  ...
  if (!state.stepping.extension.validExtensionForStep(state, *this,
                                                      state.stepping) ||
      !state.stepping.extension.k1(state, *this, state.stepping, sd.k1,
                                   sd.B_first)) {
    return 0.;
  }
  ...
  const auto tryRungeKuttaStep = [&](const double h) -> bool {
    ...
    if (!state.stepping.extension.k2(state, *this, state.stepping, sd.k2,
                                     sd.B_middle, half_h, sd.k1)) {
      return false;
    }

    // Third Runge-Kutta point
    if (!state.stepping.extension.k3(state, *this, state.stepping, sd.k3,
    ...
    if (!state.stepping.extension.k4(state, *this, state.stepping, sd.k4,
                                     sd.B_last, h, sd.k3)) {
      return false;
    }
