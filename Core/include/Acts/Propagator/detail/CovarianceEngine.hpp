...
namespace CovarianceEngine {
  ...
  ///   - and the path length (from start - for ordering)
  std::tuple<CurvilinearParameters, BoundMatrix, double> curvilinearState(StepperState& state, bool reinitialize);
  
  /// @brief Method for on-demand transport of the covariance to a new frame at current position in parameter space
  ///
  ...
  void covarianceTransport(StepperState& state, bool reinitialize, const Surface* surface = nullptr);
