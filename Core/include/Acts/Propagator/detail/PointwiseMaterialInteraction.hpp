...
struct PointwiseMaterialInteraction {
  ...
  std::pair<double, double> evaluateMultipleScattering(
      propagator_state_t& state, const stepper_t& stepper, double tInX0,
    ...
    if (state.stepping.navDir == forward) {
      ...
      state.stepping.cov(ePHI, ePHI) += state.stepping.navDir * sigmaDeltaPhiSq;
  ...
  double evaluateEnergyLossForCovariance(propagator_state_t& state,
                                         double thickness, double eLoss,
                                         double p, double lbeta) const {
    ...
    // Good in any case for positive direction
    if (state.stepping.navDir == forward) {
      state.stepping.cov(eQOP, eQOP) += state.stepping.navDir * sigmaQoverP2;
    } else {
  ...
  std::pair<double, double> evaluateEnergyLoss(
    ...
    if (state.stepping.covTransport) {
      result.second = evaluateEnergyLossForCovariance(
          state, mProperties.thickness(), eLoss.second, p, lbeta);
    }
    return result;
  }
  ...
  std::array<double, 4> evaluateMaterialInteraction(
    ...
    if (energyLoss) {
      sigmaQoP =
          evaluateEnergyLoss(state, stepper, mProperties, E, p, m, lbeta);
    }
