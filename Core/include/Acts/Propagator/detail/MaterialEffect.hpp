...
struct MultipleScattering {
  template <typename propagator_state_t, typename stepper_t,
            typename component_state_t, typename material_interaction_t>
  void operator()(propagator_state_t& /*no use*/, const stepper_t& stepper,
                  component_state_t& component_state, double sigma,
...
struct EnergyLoss {
  template <typename propagator_state_t, typename stepper_t,
            typename component_state_t, typename material_interaction_t>
  void operator()(propagator_state_t& /*nouse*/, const stepper_t& stepper,
                  component_state_t& component_state, const double p,
                  const double m, const double E, const double dE,
    ...
    if (E + dE > m) {
      ...
      stepper.update(component_state, stepper.position(component_state),
                     stepper.direction(component_state),
                     std::copysign(newP, stepper.momentum(component_state)),
                     stepper.time(component_state));
    }
