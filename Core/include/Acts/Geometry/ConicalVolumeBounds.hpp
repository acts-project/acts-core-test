...
class ConicalVolumeBounds : public VolumeBounds {
 public:
  ...
  /// @param halfPhiSector The opening angle phi sector (defaulted to M_PI)
  ConicalVolumeBounds(double innerAlpha, double innerTipZ, double outerAlpha,
                      double outerOffsetZ, double halflengthZ,
                      double averagePhi = 0., double halfPhiSector = M_PI);
 ...
 private:
  ...
  std::shared_ptr<RadialBounds> m_smallerDiscBounds = nullptr;
  std::shared_ptr<RadialBounds> m_biggerDiscBounds = nullptr;
