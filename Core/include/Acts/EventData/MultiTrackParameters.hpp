...
public:
  ...
  using ParameterWeightTrack
      = std::pair<double, ParametersT>;
  using ParameterMapWeightTrack = std::multimap<double,
                                    ParametersT,
                                    std::greater<double>>;
  ...
  MultiTrackParameters(std::list<std::pair<double, P>>&& weightParList, std::shared_ptr<multiSurfaceType> surface)
    : TrackParametersBase(),
  ...
  template <typename T = ChargePolicy,
            std::enable_if_t<std::is_same<T, ChargedPolicy>::value, int> = 0>
  MultiTrackParameters(std::list<std::pair<double, SingleCurvilinearTrackParameters<ChargedPolicy> >>&& weightParList)
    : TrackParametersBase(),
	  m_mWeightTracks(weightParList.begin(), weightParList.end()),
	  m_oChargePolicy(weightCharge()), 
	  m_oParameters(weightParameterSet()),
  ...
  MultiTrackParameters(const MultiTrackParameters<ChargePolicy, ParametersT>& copy) = default;

  /// @brief default move constructor
  MultiTrackParameters(MultiTrackParameters<ChargePolicy, ParametersT>&& copy) = default;

  /// @brief assignment operator for BoundParameters
  MultiTrackParameters<ChargePolicy, ParametersT >&
  operator=(const MultiTrackParameters<ChargePolicy, ParametersT >& rhs){
  ...
  MultiTrackParameters<ChargePolicy, ParametersT>&
  operator=(MultiTrackParameters<ChargePolicy, ParametersT>&& rhs){ 
    // check for self-assignment
    if (this != &rhs) {
  ...
  ///
  bool
  operator==(const TrackParametersBase& /*unused*/) const final
  {
    return true;
  }
  ...
  /// @brief append a single parameter to the track map for CurvilinearParameters
  void
  append(double weight, SingleCurvilinearTrackParameters<ChargedPolicy>&& para)  
  {
    m_mWeightTracks.insert(std::make_pair(weight, std::move(para)));
	m_oChargePolicy = weightCharge(); 
	m_oParameters   = getParameterSet();
  ...
  weightParameterSet() const
  {
    ...
    for (const auto& weightTrack : m_mWeightTracks) {
	  parValues += weightTrack.first *  weightTrack.second.getParameterSet().template getParameters(); 
    }
	return parSet;
  }
  ...
  size() const
  {
    return m_mWeightTracks.size();
  }

  /// @brief const getter function of the track map
  const ParameterMapWeightTrack&
  getTrackList() const
  {
  ...
  uncertainty() const
  {
    return getParameterSet().getUncertainty();
  }
