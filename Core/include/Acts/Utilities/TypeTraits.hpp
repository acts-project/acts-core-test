 ...
 * and have the compiler try to calculate that expression's resulting type, like
 * so:
 * ```
 * decltype(std::declval<T>().member_a)
 * ```
 * `std::declval<T>()` constructs a **pseudo-value** of type `T` which works
 * even if `T` is not actually constructible like this. Then we access a member
 * called `member_a` inside and instruct the compiler to calculate the resulting
 * type using `decltype`. This will only work if the expression is valid. If
 * not, this would normally cause a compilation error. If we wrap it into the
 ...
 * `Args` so that the actual evaluation of the expression happens **inside the
 * overload resolution context**. Luckily, writing a metafunction around the
 ...
 *
 * Basically, what you have to do to write type assertions using this pattern is
 * metafunctions like the one described above. Those can be member checks,
 ...
 * types. For instance, say we have two types `U` and `V`. We can then write a
 * metafunction that attempts to instantiate a binary operation between the two:
 * ```
 * template <typename U, typename V>
 * using binary_op_t = decltype(std::declval<U>() + std::declval<V>());
 * ```
 * and simply takes both `U` and `V` as meta-arguments.
 * Invoked like `is_detected<binary_op_t, TypeA, TypeB>` on some types `TypeA`
 * and `TypeB`, this will tell you whether you can call that binary operation on
 ...
 * what you gave exactly. If you want to check for a method of a given specifier
 * and you don't care if the method is const or not, you have to write out both
 * variants explicitly, and combine them with `either`.
 */
