...
Acts::KalmanVertexUpdater::updatePosition(
  ...
  oldVtxWeight = (vtx->covariance()).inverse().eval();
  ...
  newVtxCov = (oldVtxWeight +
                    trackWeight * sign * posJac.transpose() * (gBmat * posJac)).inverse();
...
Acts::Result<void> Acts::KalmanVertexUpdater::detail::update(
  ...
  double trkChi2 =
      detail::trackParametersChi2<input_track_t>(newVertexPos, trk.linearizedState, trkParamWeight);

  // Calculate new chi2
  chi2 += sign * (detail::vertexPositionChi2<input_track_t>(vtx, newVertexPos, oldVertexWeight) +
                  trackWeight * trkChi2);
