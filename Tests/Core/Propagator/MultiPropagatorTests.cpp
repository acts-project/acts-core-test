...
BOOST_DATA_TEST_CASE(
  ...
  using PropagateState = EigenPropagatorType::State<
      PropagatorOptions<ActionListType, AbortConditionsType>>;
  ...
  auto intersect1 = stepper.targetSurface(prop_state.stepping, aSurface.get(),
                                          opt, corrector);
  // calculate with the intersection estimate
  auto intersect2 = aSurface.get()->surfaceIntersectionEstimate(
      prop_state.geoContext, stepper.position(prop_state.stepping),
      stepper.direction(prop_state.stepping), opt, corrector);
  // test if targetSurface find the correct intersection
  CHECK_CLOSE_REL(intersect1.intersection.position,
                  intersect2.intersection.position, 10e-6);
  CHECK_CLOSE_REL(intersect1.intersection.pathLength,
                  intersect2.intersection.pathLength, 10e-6);
...
BOOST_DATA_TEST_CASE(
  ...
  using ActionListType = ActionList<>;
  using AbortConditionsType = AbortList<>;
  using PropagateState = MultiEigenPropagatorType::State<
      PropagatorOptions<ActionListType, AbortConditionsType>>;
  ...
  prop_state.stepping.stateCol.push_back(std::make_tuple(
      MultiEigenStepperType::SingleStateType(tgContext, mfContext, start2,
                                             forward, options.maxStepSize),
      1., StateStatus::Free));
  ...
  mcstepper.targetSurface(prop_state.stepping, aSurface.get(), opt, corrector);
  // step to surface-a
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(reference_status_check.begin(),
                                ...
                                status_check.begin(), status_check.end());
  ...
  mcstepper.targetSurface(prop_state.stepping, bSurface.get(), opt, corrector);
  ...
  prop_state.stepping.stateCol.push_back(std::make_tuple(
      MultiEigenStepperType::SingleStateType(tgContext, mfContext, start3,
                                             forward, options.maxStepSize),
      10e-6, StateStatus::Free));
  mcstepper.normalizeComponents(prop_state.stepping);
  ...
  // component is killed
  mcstepper.targetSurface(prop_state.stepping, cSurface.get(), opt, corrector);
